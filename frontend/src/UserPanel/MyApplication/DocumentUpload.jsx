...
const DocumentUpload = ({ applicationNumber, documents, handleFileUpload }) => {
  ...
  const uploadedCount = businessRequirements.filter(doc => documents[doc.id]).length;
  const requiredUploadedCount = requiredDocs.filter(doc => documents[doc.id]).length;

  return (
    <div className="bg-white rounded-xl shadow-lg border border-gray-200">
      <div className="bg-green-600 text-white p-4 sm:p-6 rounded-t-xl">
        <h2 className="text-xl sm:text-2xl font-semibold mb-2">Step 3: Business Requirements</h2>
        <p className="opacity-90 text-sm sm:text-base">Upload your documents - Complete attachments required</p>
      </div>
      ...
      <div className="p-4 sm:p-6">
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-sm text-gray-500">{Math.round((uploadedCount / businessRequirements.length) * 100)}% Complete</span>
          </div>
          <div className="w-full bg-gray-200 rounded-full h-2">
            <div 
              className="bg-green-600 h-2 rounded-full transition-all duration-500" 
              style={{ width: `${(uploadedCount / businessRequirements.length) * 100}%` }}
            ></div>
          </div>
        </div>
          ...
          <div className="space-y-4">
            {requiredDocs.map((doc, index) => (
              <div key={doc.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-all">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="flex items-start space-x-4">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs sm:text-sm text-gray-500 mt-1">{doc.description}</p>
                    </div>
                  </div>
                ...
                {documents[doc.id] && (
                  <div className="mt-3 ml-12 p-2 bg-green-50 rounded text-sm text-green-700">
                    📎 {documents[doc.id].name} ({Math.round(documents[doc.id].size / 1024)} KB)
                  </div>
                )}
              </div>
            ))}
        ...
        <div className="mb-6">
          ...
          <div className="space-y-4">
            {optionalDocs.map((doc, index) => (
              <div key={doc.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-all opacity-75">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="flex items-start space-x-4">
                    ...
                    <div className="min-w-0">
                      <h4 className="font-semibold text-gray-800 text-sm sm:text-base">
                        {doc.name}
                        <span className="text-blue-500 ml-1 text-xs">(Optional)</span>
                      </h4>
                      <p className="text-xs sm:text-sm text-gray-500 mt-1">{doc.description}</p>
                    </div>
                  </div>
          ...
          <div className="flex items-start">
            ...
            <div className="text-sm text-yellow-800">
              ...
              <ul className="space-y-1 text-xs sm:text-sm">
                ...
                <li>• Required documents marked with (*) are mandatory for processing</li>
                <li>• Optional documents should be uploaded if applicable to your business</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-center">
            <div className="bg-white rounded-lg p-3 border">
              <div className="text-2xl font-bold text-gray-800">{uploadedCount}</div>
              <div className="text-sm text-gray-600">Total Uploaded</div>
            </div>
            <div className="bg-white rounded-lg p-3 border">
              <div className={`text-2xl font-bold ${requiredUploadedCount === requiredDocs.length ? 'text-green-600' : 'text-red-600'}`}>
                {requiredUploadedCount}/{requiredDocs.length}
              </div>
              <div className="text-sm text-gray-600">Required Documents</div>
            </div>
            <div className="bg-white rounded-lg p-3 border">
              <div className="text-2xl font-bold text-blue-600">{optionalDocs.filter(doc => documents[doc.id]).length}</div>
